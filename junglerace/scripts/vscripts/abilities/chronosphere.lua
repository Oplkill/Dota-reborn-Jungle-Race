...
function ChronosphereAura( keys )
	...
	--print("[DEBUG Chronosphere] Aura on " .. target:GetUnitName())
	...
	--[[if (caster:GetPlayerOwner() == target:GetPlayerOwner()) or (target:GetUnitName() == "unit_faceless" and ignore_void) then
--		target:AddNewModifier(caster, ability, "modifier_chronosphere_speed_lua", {duration = duration})
		print("[DEBUG Chronosphere] Aura on FRIEND")
	else]]
	-- Everyone else gets immobilized and stunned
		--print("[DEBUG Chronosphere] STUNNING ENEMY" .. target:GetUnitName())
